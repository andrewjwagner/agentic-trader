...
export interface Holding {
  ...
  entryPrice: number;
  /** Baked mark for static hosting when live quotes fail. */
  lastPrice: number;
  score: number;
  sleeve: Sleeve;
  notes?: string;
...
export function costBasisTotal(p: PortfolioSnapshot = portfolio): number {
  return p.holdings.reduce((s, h) => s + h.shares * h.entryPrice, 0);
}

export function bakedQuotes(p: PortfolioSnapshot = portfolio): Record<string, number> {
  const map: Record<string, number> = {};
  for (const h of p.holdings) map[h.symbol] = h.lastPrice;
  return map;
}
